...
class MiniMax:
    def __init__(self, depth):
        ...
        self.maxDepth = depth
        self.gmo = GenerateMovesMidGameEndGame()
        self.maxPos = ""
    ...
    def staticEstimation(self, boardPos : str)->int:
        ...
        blackMoves = self.gmo.generate(self.gmo.reversePosition(boardPos))
        ...
        if numBlackPieces <= 2 : 
            ...
            whiteMills, blackMills = self.gmo.getNumCloseMills(boardPos)
            whiteMills *= 2
            blackMills *= 2
            whiteDefenders, blackDefenders = self.gmo.getDefendingPos(boardPos) 
            whiteDefenders *= 2
            blackDefenders *= 2
            for move in blackMoves:
                bC, wC = self.gmo.getNumCloseMills(move)
                bD, wD = self.gmo.getDefendingPos(move)
                blackMills += bC
                whiteMills += wC
                whiteDefenders += wD
                blackDefenders += bD            
            #return ((1000 * (numWhitePieces - numBlackPieces) ) - numBlackMoves)
            return ((10 * ((5 * numWhitePieces + 3 * whiteMills + 2 * whiteDefenders) - \
                (5 * numBlackPieces + 3 * blackMills + 2 * blackDefenders))) - 2 * numBlackMoves)
    

class GenerateMovesMidGameEndGame:
    """
    Input: A board Position
    ...
    def __init__(self):
        ...
        self.neigh = {0 : [1, 2, 6], 1: [0, 3, 11], 2: [0, 3, 4, 7], 3: [1, 2, 5, 10],\
                        4 : [2, 5, 8], 5 : [3, 4, 9], 6 : [0, 7, 18], 7 : [2, 6, 8, 15], \
                            8 : [4, 7, 12], 9 : [5, 10, 14], 10 : [3, 9, 11, 17], \
                                11: [1, 10, 20], 12 : [8, 13, 15], 13: [12, 14, 16], \
                                    14 : [9, 13, 17], 15 : [7, 12, 16, 18], 16 : [13, 15, 17, 19], \
                                        17 : [10, 14, 16, 20], 18 : [6, 15, 19], 19 : [16, 18, 20], \
                                            20 : [11, 17, 19]}

    def generate(self, currBoardPos):
        self.res = []
        numWhite = currBoardPos.count('W')
        if numWhite == 3:
            self.generateHopping(currBoardPos)
        else:
            self.generateMove(currBoardPos)
        return self.res
    ...
    def generateHopping(self, currBoardPos):
        N = len(currBoardPos)
        for idx1 in range(N):
            if currBoardPos[idx1] == 'W':
                for idx2 in range(N):
                    if currBoardPos[idx2] == 'x':
                        newBoardPos = ""
                        if idx1 < idx2:
                            newBoardPos = currBoardPos[:idx1]+ 'x' + currBoardPos[idx1 + 1: idx2] + 'W' + currBoardPos[idx2 + 1:]
                        else:
                            newBoardPos = currBoardPos[:idx2]+ 'W' + currBoardPos[idx2 + 1: idx1] + 'x' + currBoardPos[idx1 + 1:]
                        if self.closeMill(idx2, newBoardPos):
                            self.generateRemove(newBoardPos)
                        else:
                            self.res.append(newBoardPos)


    def generateMove(self, currBoardPos):

        for idx, ch in enumerate(currBoardPos):
            if ch == 'W':
                nei = self.neighbors(idx)
                for n in nei:
                    if currBoardPos[n] == 'x':
                        newBoardPos = ""
                        if idx < n:
                            newBoardPos = currBoardPos[:idx]+ 'x' + currBoardPos[idx + 1: n] + 'W' + currBoardPos[n + 1:]
                        else:
                            newBoardPos = currBoardPos[:n]+ 'W' + currBoardPos[n + 1: idx] + 'x' + currBoardPos[idx + 1:]

                        if self.closeMill(n, newBoardPos):
                            self.generateRemove(newBoardPos)
                        else:
                            self.res.append(newBoardPos)
    ...
    def getDefendingPos(self, boardPos:str)-> int:
        numBlackDefending, numWhiteDefending = 0, 0
        for idx in range(len(boardPos)):
            for pos1, pos2 in self.millPos[idx]:
                if boardPos[idx] == 'W' and boardPos[pos1] == boardPos[pos2] == 'B':
                    numWhiteDefending += 1
                elif boardPos[idx] == 'B' and boardPos[pos1] == boardPos[pos2] == 'W':
                    numBlackDefending += 1
        return (numWhiteDefending, numBlackDefending)
